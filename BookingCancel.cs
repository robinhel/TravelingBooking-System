...
public static class BookingCancel      // "Partial" allows this class to be split across multiple files
{
    public static async Task<IResult> CancelBooking(int bookingId, Config config, HttpContext ctx)
    {
        ...
        using var transction = await connection.BeginTransactionAsync(); 

        try
        {
            // Delete room-booking dependants first; foregin key constraints
            ...
            using (var command = new MySqlCommand(deleteRoomLinks, connection, (MySqlTransaction) transction))
            {
                command.Parameters.AddWithValue("@booking_id", bookingId);   // Bind the booking ID parameter
                await command.ExecuteNonQueryAsync();                        // Execute deletion of lnked room
            }
            ...
            using (var command = new MySqlCommand(deleteBooking, connection, (MySqlTransaction) transction))
            {
                ...
                if (affected == 0)
                {
                    await transction.RollbackAsync();        // Undo previous delete
        ...
        catch (Exception ex)
        {
            ...
            return Results.Problem($"Error cancelling booking: {ex.Message}");
        }
    }
}